...
def add_markup(story, answers, current_turn, history_len):
    # Add markup for current turn and history_len previous turns
    marked_story = story
    turn_count = len(answers)
    for i in range(max(current_turn - history_len, -1), current_turn + 1):
        if i >= 0:
            span_text = answers[i]["span_text"]
            turn_id = answers[i]["turn_id"]

            # Add markup to story
            marked_story = marked_story.replace(span_text, f"<{turn_id}>{span_text}<{turn_id}>")
...
def main():
    ...
    # 新JSON文件的保存位置
    output_file = './finetune_data/v3/add_prompts/tiqa-train-v3-llm-h0-forward.json'
    # output_file = 'tiqa-dev-v3-test-hlf.json'
    # 设置 history_len
